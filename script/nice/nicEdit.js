...
var nicEditorConfig = bkClass.extend({
	buttons : {
		...
		'left' : {name : __('Alinhar à Esquerda'), command : 'justifyleft', noActive : true},
		'center' : {name : __('Alinhar ao Centro'), command : 'justifycenter', noActive : true},
		'right' : {name : __('Alinhar à Direita'), command : 'justifyright', noActive : true},
		'justify' : {name : __('Justificado'), command : 'justifyfull', noActive : true},
		'ol' : {name : __('Inseir Lista Ordenada'), command : 'insertorderedlist', tags : ['ol']},
		'ul' : 	{name : __('Inserir lista Não Ordenada'), command : 'insertunorderedlist', tags : ['ul']},
		'subscript' : {name : __('Subscrito'), command : 'subscript', tags : ['sub']},
		'superscript' : {name : __('Sobrescrito'), command : 'superscript', tags : ['sup']},
		'strikethrough' : {name : __('Tachado'), command : 'strikeThrough', css : {'text-decoration' : 'line-through'}},
		'removeformat' : {name : __('Remover Formatação'), command : 'removeformat', noActive : true},
		'indent' : {name : __('Aumentar Recuo'), command : 'indent', noActive : true},
		'outdent' : {name : __('Diminuir Recuo'), command : 'outdent', noActive : true},
		'hr' : {name : __('Linha Horizontal'), command : 'insertHorizontalRule', noActive : true}
	},
	iconsPath : 'script/nice/nicEditorIcons.gif',
...
var nicEditors={nicPlugins:[],editors:[],registerPlugin:function(B,A){this.nicPlugins.push({p:B,o:A})},allTextAreas:function(C){var A=document.getElementsByTagName("textarea");
for(var B=0;B<A.length;B++){
	nicEditors.editors.push(new nicEditor(C).panelInstance(A[B]))}return nicEditors.editors},findEditor:function(C){var B=nicEditors.editors;
...
var nicLinkOptions = {
	buttons : {
		'link' : {name : 'Hiperlink', type : 'nicLinkButton', tags : ['A']},
		'unlink' : {name : 'Remover Hiperlink',  command : 'unlink', noActive : true}
	}
};

var nicLinkButton=nicEditorAdvancedButton.extend({addPane:function(){this.ln=this.ne.selectedInstance.selElm().parentTag("A");this.addForm({"":{type:"title",txt:"Add/Edit Link"},href:{type:"text",txt:"URL",value:"http://",style:{width:"150px"}},title:{type:"text",txt:"Title"},target:{type:"select",txt:"Open In",options:{"":"Current Window",_blank:"New Window"},style:{width:"100px"}}},this.ln)},submit:function(C){var A=this.inputs.href.value;if(A=="http://"||A==""){alert("Entre com a URL da imagem");return false}this.removePane();if(!this.ln){var B="javascript:nicTemp();";this.ne.nicCommand("createlink",B);this.ln=this.findElm("A","href",B)}if(this.ln){this.ln.setAttributes({href:this.inputs.href.value,title:this.inputs.title.value,target:this.inputs.target.options[this.inputs.target.selectedIndex].value})}}});nicEditors.registerPlugin(nicPlugin,nicLinkOptions);


var nicColorOptions = {
	buttons : {
		'forecolor' : {name : __('Mudar a cor do texto'), type : 'nicEditorColorButton', noClose : true},
		'bgcolor' : {name : __('Mudar a imagem de fundo'), type : 'nicEditorBgColorButton', noClose : true}
	}
};
...
var nicImageButton=nicEditorAdvancedButton.extend({addPane:function(){
															...
															this.addForm({"":{type:"title",txt:"Adicionar/Editar Imagem <br /> <a href=\"#\" onClick=\"window.open('?acao=bibliotecaAction&pg=Listar', 'Janela', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=960, height=300'); return false;\">Da Biblioteca</a>"},src:{type:"text",txt:"URL",value:"http://",style:{width:"150px"}},alt:{type:"text",txt:"Alt Text",style:{width:"100px"}},align:{type:"select",txt:"Alinhamento",options:{left:"Esquerda",right:"Direita", none:"Nenhuma"}}},this.im)},submit:function(B){var C=this.inputs.src.value;
															if(C==""||C=="http://"){
																alert("Entre com a URL da imagem");
																return false
															}												
...
var nicSaveOptions = {
	buttons : {
		'save' : {name : __('Salvar este Conteudo'), type : 'nicEditorSaveButton'}
	}
};
...
var nicUploadOptions = {
	buttons : {
		'upload' : {name : 'Upload de Imagem', type : 'nicUploadButton'}
	}
	
};

var nicUploadButton=nicEditorAdvancedButton.extend({
		nicURI:"controlador/uploadBiblioteca.php",
		addPane:function(){
			this.im=this.ne.selectedInstance.selElm().parentTag("img");
			this.myID="tibapa";
			...
			this.myFrame=new bkElement("iframe").setAttributes({
				...
				frameBorder:0,
				scrolling:"no"
			}).setStyle({border:0}).appendTo(this.pane.pane);
	...
	setTimeout(this.addForm.closure(this),50)},addForm:function(){var A=this.myDoc=this.myFrame.contentWindow.document;
	A.open();
	A.write("<html><body>");
	A.write('<form method="post" action="'+this.uri+'" enctype="multipart/form-data">');
	A.write('<input type="hidden" name="APC_UPLOAD_PROGRESS" value="'+this.myID+'" />');
	if(this.uri==this.nicURI){A.write('<div style="position: absolute; margin-left: 160px;"></div>')}A.write('<div style="font-size: 14px; font-weight: bold; padding-top: 5px;">Insira uma Imagem</div>');
	...
	this.myStatus.setContent('<img src="script/images/load.gif" style="float: right; margin-right: 40px;" /><strong>Enviando...</strong><br />Aguarde');
	this.myForm.submit();	
	setTimeout(		
		this.makeRequest.closure(this),
	...
	update:function(C){
		...
		if(C.url){
			...
			if(C.error){
				this.requestInterval=false;
				this.setProgress(100);
				alert("Erro ao fazer Upload:  ("+C.error+").");
